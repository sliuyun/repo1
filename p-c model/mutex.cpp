...
/**
  * @brief 
  * @param 如果用于互斥锁，value填1，sem_flg填SEM_UNDO
  * @param 如果用于生产消费者模型，value填0，sem_flg填0
  * @retval 
  */
bool csemp::init(key_t key,unsigned short value,short sem_flg)
{
    ...
    // 信号量的初始化不能直接用在semget(key,1,0666 | IPC_CREAT)
    // 因为信号量创建后，初始值是0，如果用于互斥锁，需要把他初始化设置为1
    ...
    else{perror("init 3 semget()"); return false;}

    return true;
}

// 信号量的P操作（把信号量的值加value），如果信号值是0，将阻塞等待，直到信号值的值大于0
...
bool csemp::destory()
{
    ...
    if(semctl(m_semid,0,IPC_RMID) == -1) {perror("destory semctl()"); return false;  }
